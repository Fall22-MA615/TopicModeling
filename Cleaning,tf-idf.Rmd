...
tf_idf_high %>% 
  filter(review_index %in%  c(3465,5044,5140,13797,25247)) %>%
  arrange(desc(tf_idf)) %>%
  ...
  labs(title = "Highest tf-idf words in whole reviews",
       ...
       x = "tf-idf", y = NULL)
```
On each 6 plot, we can see top 15 words with high tf-idf. Among them, we can verify some meaningful words for checking their genres. For example, in review'3465', the words 'horror','devil's' imply that the review is about horror movie.